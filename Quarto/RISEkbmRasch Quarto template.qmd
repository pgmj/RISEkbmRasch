...
# # this method throws "Error in `recode()`: ! Argument 2 must be named."
# # thought it was a namespace collision, but neither appending car:: nor dplyr:: fixes it
# 
...
# scale.items <- df %>%
#   dplyr::select(starts_with("WAAQ")) %>% # this selects all columns/variables that start with WAAQ
#   names()
# 
...
scale.items <- paste0("WAAQ_", seq(from = 0, to = 6, by = 1))

df <- df %>%
  dplyr::rename(WAAQ_0 = WAAQ_1,
                WAAQ_1 = WAAQ_2,
                WAAQ_2 = WAAQ_3,
                WAAQ_3 = WAAQ_4,
                WAAQ_4 = WAAQ_5,
